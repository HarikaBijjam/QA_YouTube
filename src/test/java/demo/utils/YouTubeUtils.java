...
public class YouTubeUtils {
    /**
     * Logs a status message with the specified method name and message to the
     * console.
     *
    ...
    /**
     * Logs a status message with the specified test case ID, test step, and test
    ...
    public static void logStatus(String testCaseID, String testStep, String testMessage) {
        System.out.println(String.format("\n%s | %s | %s | %s\n", getDateTime("yyyy-MM-dd HH:mm:ss"), testCaseID,
                testStep, testMessage));
    }

    /**
     * Retrieves the current date and time in the specified format pattern.
     *
     * @param formatPattern The pattern used to format the date and time (e.g.,
     *                      "yyyy-MM-dd HH:mm:ss").
     * @return A string representing the current date and time formatted according
     ...
     *
     * @param driver        The WebDriver instance whose current URL is to be
     *                      verified.
    ...
    /**
     * Scrolls the viewport to make the specified element visible within the
     * WebDriver's current viewport.
     *
     * @param driver  The WebDriver instance on which the scrolling operation is
     *                performed.
    ...
    /**
     * Retrieves and prints the displayed message of an element identified by the
     * specified locator.
     *
     * @param driver  The WebDriver instance where the element is located.
     * @param locator The By locator used to identify the element containing the
     *                displayed message.
     */
    ...
    /**
     * Scrolls to the extreme end of the page by continuously clicking on an element
     * identified by the specified locator.
     *
     * @param driver  The WebDriver instance to perform the scrolling operation.
     * @param locator The By locator used to identify the element to click for
     *                scrolling.
     */
    ...
    /**
     * Retrieves a WebElement identified by the specified locator within the
     * WebDriver's current context.
     *
     * @param driver         The WebDriver instance where the element is located.
     * @param elementLocator The By locator used to identify the WebElement to
     *                       retrieve.
     * @return The WebElement identified by the locator if found and visible within
     *         the specified timeout, otherwise returns null.
     */
    ...
    /**
     * Retrieves the text content of a child element located within a parent element
     * identified by the provided locators.
     *
     ...
     * @param childLocator   The By locator used to identify the child element
     *                       within the parent element.
     * @return The text content of the child element if found, otherwise an empty
     *         string.
     */
    ...
    /**
     * Retrieves a list of WebElements identified by the specified locator within
     * the WebDriver's current context.
     *
     * @param driver            The WebDriver instance where the elements are
     *                          located.
     * @param firstNNewsLocator The By locator used to identify the list of
     *                          WebElements to retrieve.
     * @return A list of WebElements identified by the locator if found, otherwise
     *         an empty list.
     */
    ...
    /**
     * Retrieves the body content and view count of news posts identified by the
     * specified locator within the WebDriver's current context.
     *
     * @param driver            The WebDriver instance where the news posts are
     *                          located.
     * @param firstNNewsLocator The By locator used to identify the list of news
     *                          posts.
     */
        ...
        try {
            ...
            for (WebElement elementBody : newsPosts) {
                ...
                try {
                    ...
                    WebElement numberOfLikes = elementBody
                            .findElement(By.xpath(".//span[contains(@id, 'vote-count-middle')]"));
    ...
    /**
     * Scrolls through video views until the total count reaches the specified
     * target count.
     *
    ...
    public static void scrollTillVideoCountReaches(WebDriver driver, long totalCount) {
        try {
            ...
            while (totalCount >= 0) {
                ...
                WebElement videoViews = wait.until(ExpectedConditions.visibilityOfElementLocated(
                        By.xpath("(//ytd-video-renderer//span[contains(text(), 'views')])[" + index + "]")));

                // Scroll the viewport to the video view element
                YouTubeUtils.scrollToViewport(driver,
                        By.xpath("(//ytd-video-renderer//span[contains(text(), 'views')])[" + index + "]"));
                ...
                index++;
                logStatus("scrollTillVideoCountReaches",
                        String.format("(%s) %s views, Remaining: %s", videoViews.getText(),
                                parseViewsCount(videoViews.getText()),
                                (totalCount <= 0) ? "Count reached" : totalCount));
            }
        } catch (Exception e) {
